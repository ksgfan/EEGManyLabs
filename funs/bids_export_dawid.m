...
function bids_export_dawid(files, varargin)
...
opt = finputcheck(varargin, {
    ...
    'exportformat' 'string'  {'same' 'eeglab' 'edf' 'bdf' 'vhdr'}    'eeglab';
...
opt.eInfo = {};
% onset
opt.eInfo{1,1} = 'onset'; opt.eInfo{1,2} = 'latency';
opt.eInfoDesc.onset.Description = 'Onset (in seconds) of the event from the beginning of the first datapoint.';
opt.eInfoDesc.onset.Units = 's';
% duration
opt.eInfo{2,1} = 'duration'; opt.eInfo{2,2} = 'duration';
opt.eInfoDesc.duration.Description = 'Duration of the event in seconds from onset.';
opt.eInfoDesc.duration.Units = 's';
% type
i_value = find(ismember(ev_names, 'value'));
opt.eInfo{3, 1} = ev_names{i_value}; opt.eInfo{3, 2} = ev_names{i_value};
opt.eInfoDesc.(ev_names{i_value}).Description = 'The event code (also known as trigger code or event ID) associated with the event.';
% latency
...
if any(ismember(ev_names, 'sac_amplitude'))
    ...
    for et_n = 1 : length(et_names)
        opt.eInfo{end+1, 1} = et_names{et_n}; opt.eInfo{end, 2} = et_names{et_n};
...
for iRow=1:size(matlabArray,1)
    for iCol=1:size(matlabArray,2)
        if isempty(matlabArray{iRow,iCol}) %%% DAWID|| all(isnan(matlabArray{iRow,iCol}))
