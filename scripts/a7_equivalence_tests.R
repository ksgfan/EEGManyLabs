...
# load data
dat = read.csv(file.path(project_path, 'data/csv_files/all_stats_direct.csv'), header = T)

dat = read.csv(file.path(project_path, 'data/csv_files/all_stats_advanced.csv'), header = T)

dat = read.csv(file.path(project_path, 'data/csv_files/all_stats_ica.csv'), header = T)

dat = read.csv(file.path(project_path, 'data/csv_files/all_stats_keep_all.csv'), header = T)
labs = dat$Lab
...
save(cohens_d_h1.3, cohens_d_se_h1.3, r_h2.2, r_se_h2.2, 
     file = file.path(project_path, 'data/csv_files/eq_test_results.Rda'))
