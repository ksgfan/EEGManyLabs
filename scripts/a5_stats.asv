...
for pip = 1 : 1%length(pipelines)
    ...
    for lab = 1 : length(labs)
        ...
        [h,p,ci,stats] = ttest(contra, ipsi);
        cohensd = computeCohen_d(contra, ipsi, 'paired');
        % compute standard error of cohen's d (for paired test)
        r = corr(contra, ipsi);
        cohensd_se = sqrt( ( (2*(1-r)) / length(contra) ) + ( cohensd^2 / (2 * length(contra)) ) );
        ...
        all_stats.on_t_stat(lab) = stats.tstat;
        all_stats.on_df(lab) = stats.df;
        all_stats.on_sd(lab) = stats.sd;
        all_stats.on_p(lab) = p;
        all_stats.on_ci1(lab) = ci(1);
        all_stats.on_ci2(lab) = ci(2);
        all_stats.on_cohensd(lab) = cohensd;
        all_stats.on_cohensd_se(lab) = cohensd_se;
        ...
        [h,p,ci,stats] = ttest(m_corr, m_incorr);
        cohensd = computeCohen_d(m_corr, m_incorr, 'paired');
        % https://real-statistics.com/students-t-distribution/paired-sample-t-test/cohens-d-paired-samples/
        r = corr(m_corr, m_incorr);
        cohensd_se = sqrt( ( (2*(1-r)) / length(m_corr) ) + ( cohensd^2 / (2 * length(m_corr)) ) );
        ...
        all_stats.corrincorr_t_stat(lab) = stats.tstat;
        all_stats.corrincorr_df(lab) = stats.df;
        all_stats.corrincorr_sd(lab) = stats.sd;
        all_stats.corrincorr_p(lab) = p;
        all_stats.corrincorr_ci1(lab) = ci(1);
        all_stats.corrincorr_ci2(lab) = ci(2);
        all_stats.corrincorr_cohensd(lab) = cohensd;
        all_stats.corrincorr_cohensd_se(lab) = cohensd_se;
        ...
        [h,p,ci,stats] = ttest(meanROI2, meanROI4);
        % effect sizes
        cohensd = computeCohen_d(meanROI2, meanROI4, 'paired');
        r = corr(meanROI2, meanROI4);
        cohensd_se = sqrt( ( (2*(1-r)) / length(meanROI2) ) + ( cohensd^2 / (2 * length(meanROI2)) ) );
        % save stats
        all_stats.ph_2_4_t_stat(lab, 1) = stats.tstat;
        all_stats.ph_2_4_df(lab) = stats.df;
        all_stats.ph_2_4_sd(lab) = stats.sd;
        all_stats.ph_2_4_p(lab) = p;
        all_stats.ph_2_4_ci1(lab) = ci(1);
        all_stats.ph_2_4_ci2(lab) = ci(2);
        all_stats.ph_2_4_cohensd(lab) = cohensd;
        all_stats.ph_2_4_cohensd_se(lab) = cohensd_se;
        
        % post-hoc t-tests: 2 vs 6
        [h,p,ci,stats] = ttest(meanROI2, meanROI6);
        % effect sizes
        cohensd = computeCohen_d(meanROI2, meanROI6, 'paired');
        r = corr(meanROI2, meanROI6);
        cohensd_se = sqrt( ( (2*(1-r)) / length(meanROI2) ) + ( cohensd^2 / (2 * length(meanROI2)) ) );
        % save stats
        all_stats.ph_2_6_t_stat(lab, 1) = stats.tstat;
        all_stats.ph_2_6_df(lab) = stats.df;
        all_stats.ph_2_6_sd(lab) = stats.sd;
        all_stats.ph_2_6_p(lab) = p;
        all_stats.ph_2_6_ci1(lab) = ci(1);
        all_stats.ph_2_6_ci2(lab) = ci(2);
        all_stats.ph_2_6_cohensd(lab) = cohensd;
        all_stats.ph_2_6_cohensd_se(lab) = cohensd_se;
        
        % post-hoc t-tests
        [h,p,ci,stats] = ttest(meanROI4, meanROI6);
        % effect sizes
        cohensd = computeCohen_d(meanROI4, meanROI6, 'paired');
        r = corr(meanROI4, meanROI6);
        cohensd_se = sqrt( ( (2*(1-r)) / length(meanROI4) ) + ( cohensd^2 / (2 * length(meanROI4)) ) );
        % save stats
        all_stats.ph_4_6_t_stat(lab, 1) = stats.tstat;
        all_stats.ph_4_6_df(lab) = stats.df;
        all_stats.ph_4_6_sd(lab) = stats.sd;
        all_stats.ph_4_6_p(lab) = p;
        all_stats.ph_4_6_ci1(lab) = ci(1);
        all_stats.ph_4_6_ci2(lab) = ci(2);
        all_stats.ph_4_6_cohensd(lab) = cohensd;
        all_stats.ph_4_6_cohensd_se(lab) = cohensd_se;
