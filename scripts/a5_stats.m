...
for pip = 1 : length(pipelines)
    ...
    disp([mean(tab.K6), std(tab.K6)])
    disp([mean((tab.K2+tab.K4+tab.K6)/3),     std((tab.K2+tab.K4+tab.K6)/3)])
    
    % print average Ds
    disp([mean(tab.d_prime2), std(tab.d_prime2)])
    disp([mean(tab.d_prime4), std(tab.d_prime4)])
    disp([mean(tab.d_prime6), std(tab.d_prime6)])
    disp([mean((tab.d_prime2+tab.d_prime4+tab.d_prime6)/3),     std((tab.d_prime2+tab.d_prime4+tab.d_prime6)/3)])
    ...
    for lab = 1 : length(labs)
        ...
        memoryCapacity_d = (lab_tab.d_prime2 + lab_tab.d_prime4 + lab_tab.d_prime6) / 3; % average over all setsizes
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % correlation between amplitude increase (set size 2 to 4) and memory capacity K
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        % save stats
        all_stats.wm_corr_6_amp_m(lab) = nanmean(meanROI6);
        all_stats.wm_corr_6_amp_sd(lab) = nanstd(meanROI6);
        all_stats.wm_corr_6_wm_m(lab) = nanmean(memoryCapacity);
        all_stats.wm_corr_6_wm_sd(lab) = nanstd(memoryCapacity);
        all_stats.wm_corr_6_r(lab) = rho;
        all_stats.wm_corr_6_p(lab) = pval;
        all_stats.wm_corr_6_r_se(lab) = rho_se;
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % correlation between amplitude increase (set size 2 to 4) and memory capacity D
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        [rho, pval] = corr(ampIncrease_2_to_4, memoryCapacity_d);
        % compute SE for correlations
        rho_se = (1-rho^2) / sqrt(length(ampIncrease_2_to_4) - 3);
        % save stats
        all_stats.wm_d_corr_2_4_amp_m(lab) = nanmean(ampIncrease_2_to_4);
        all_stats.wm_d_corr_2_4_amp_sd(lab) = nanstd(ampIncrease_2_to_4);
        all_stats.wm_d_corr_2_4_wm_m(lab) = nanmean(memoryCapacity);
        all_stats.wm_d_corr_2_4_wm_sd(lab) = nanstd(memoryCapacity);
        all_stats.wm_d_corr_2_4_r(lab) = rho;
        all_stats.wm_d_corr_2_4_p(lab) = pval;
        all_stats.wm_d_corr_2_4_r_se(lab) = rho_se;
